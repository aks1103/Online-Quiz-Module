...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST)){
  ...
  $sql = 'INSERT INTO participants_hunt '.'( name, email, phone ) '.'VALUES ("' . $name . '","' . $email . '","' . $phone . '")';


  if ($db->query($sql) === TRUE) {
      

   $sql = 'SELECT id  FROM participants_hunt WHERE email = "'.$email.'"';
   ...
   if ($retval->num_rows == 1) {
      while($row = $retval->fetch_assoc()) {
          ...
          $confirmlink = "http://hunt.reflux.in/confirm.php?e=$email&c=$confirm";

          $sql = 'UPDATE participants_hunt SET isActive ="'.$confirm.'" , password = "'.$pass[0].'" WHERE id = "'.$id.'"';
