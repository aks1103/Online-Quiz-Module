...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST)){
  ...
  $sql = 'INSERT INTO participants_gquiz '.'( name, email, phone ) '.'VALUES ("' . $name . '","' . $email . '","' . $phone . '")';


  if ($db->query($sql) === TRUE) {
      

   $sql = 'SELECT id  FROM participants_gquiz WHERE email = "'.$email.'"';
   ...
   if ($retval->num_rows == 1) {
      while($row = $retval->fetch_assoc()) {
          ...
          $sql = 'UPDATE participants_gquiz SET isActive ="'.$confirm.'" , password = "'.$pass[0].'" WHERE id = "'.$id.'"';
