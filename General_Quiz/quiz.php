<?php
   ...
   $sql = "SELECT isEnd FROM participants_gquiz WHERE email = '$login_session'";
   $isEnd = 0;
   $retval = $db->query($sql);
   ...
   $sql = "SELECT isStarted FROM participants_gquiz WHERE email = '$login_session'";
   $isStarted = 1;
   $retval = $db->query($sql);
    if($retval->num_rows > 0 ){
...
<center><h2 style='color:#229955'>Question $j</h2></center>
		<div class="question"  >
			...
			<img src="./questions/$questions[$i].PNG" /><br/><br/>
